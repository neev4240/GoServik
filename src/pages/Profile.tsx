  ...
  if (!pro) {
    return (
      <div className="flex h-[50vh] flex-col items-center justify-center text-center px-4">
        <h2 className="text-2xl font-bold text-slate-900">{t('proNotFoundTitle')}</h2>
        <p className="mt-2 text-slate-500">{t('proNotFoundDesc')}</p>
        <Button asChild className="mt-6">
          <Link to="/explore">{t('proBackToDirectory')}</Link>
        </Button>
      </div>
    );
  }
        ...
        <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between text-sm text-slate-500 mb-6">
            <div className="flex items-center">
              <Link to="/explore" className="hover:text-slate-900">{t('profileDirectory')}</Link>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {pro.verified && (
                      <ShieldCheck className="h-6 w-6 text-blue-500" title={t('profileVerifiedPro')} />
                    )}
                  </div>
                  {pro.personalName && (
                    <p className="text-xs font-semibold text-indigo-600 bg-indigo-50/50 border border-indigo-100/50 rounded-full px-2.5 py-1 inline-block mb-2">
                      {t('proProprietor')}: {pro.personalName}
                    </p>
                  )}
                  ...
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-4 text-sm text-slate-600">
                    <span className="flex items-center gap-1">
                      ...
                      <span className="underline decoration-slate-300">({pro.reviewCount} {t('reviewsCount')})</span>
                    </span>
                    <span className="flex items-center gap-1">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {pro.jobsCompleted} {t('completedJobs')}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto shrink-0 mt-4 sm:mt-0">
                  <Button 
                    ...
                    onClick={() => {
                      if (!currentUser) alert(t('profilePleaseSignIn'));
                      else toggleSavedProfessional(pro.id);
                    }}
                  >
                    <Heart className={`h-4 w-4 ${isSaved ? 'fill-red-500 text-red-500' : ''}`} />
                    {isSaved ? t('profileSaved') : t('profileSave')}
                  </Button>
                  {currentUser?.role === 'professional' ? (
                    <Button disabled className="flex-1 sm:flex-none bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200">
                      {t('profileBookingRestricted')}
                    </Button>
                  ) : (
                    <Button asChild className="flex-1 sm:flex-none">
                      <Link to={`/book/${pro.id}`}>{t('profileBookServices')}</Link>
                    </Button>
                  )}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-white/40 shadow-sm hover:shadow-lg transition-all">
              <h2 className="text-xl font-bold text-slate-900 mb-4">{t('profileAboutMe')}</h2>
            ...
            <section className="bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-white/40 shadow-sm hover:shadow-lg transition-all">
              <h2 className="text-xl font-bold text-slate-900 mb-4">{t('profileServicesOffered')}</h2>
                  ...
                  return (
                    <div key={service.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border bg-slate-50/50">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-2">{service.experienceYears} {t('profileExperience')}</p>
                      </div>
                      <div className="mt-4 sm:mt-0 sm:text-right shrink-0">
                        <div className="font-bold text-lg text-slate-900">
                          ₹{service.basePrice}
                          <span className="text-sm font-normal text-slate-500">
                            {service.priceUnit === 'hourly' ? t('profileHourly') : service.priceUnit === 'starting_at' ? t('profileStartingAt') : t('profileFixed')}
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            {pro.gallery && pro.gallery.length > 0 && (
              <section className="bg-white/60 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-white/40 shadow-sm hover:shadow-lg transition-all">
                <h2 className="text-xl font-bold text-slate-900 mb-4">{t('profilePortfolio')}</h2>
          ...
          <div className="space-y-6">
            <div className="bg-white/60 backdrop-blur-md p-6 rounded-3xl border border-white/40 shadow-sm hover:shadow-lg transition-all">
              <h3 className="font-bold text-slate-900 mb-4">{t('profileDetails')}</h3>
              <ul className="space-y-4 text-sm">
                <li className="flex items-start gap-3 text-slate-600">
                  <MessageSquare className="h-5 w-5 text-slate-400 shrink-0" />
                  <div>
                    <span className="block font-medium text-slate-900">{t('profileResponseTime')}</span>
                    {pro.responseTime}
                  </div>
                </li>
                <li className="flex items-start gap-3 text-slate-600">
                  ...
                  <div>
                    <span className="block font-medium text-slate-900">{t('profileServiceArea')}</span>
                    {t('profileUpTo')} {pro.serviceRadiusKm}km {t('profileFrom')} {pro.location}
                  </div>
                </li>
              </ul>
              
              <div className="mt-6 pt-6 border-t">
                <h4 className="font-medium text-slate-900 mb-2">{t('profileLanguages')}</h4>
              ...
              {pro.certifications.length > 0 && (
                <div className="mt-6 pt-6 border-t">
                  <h4 className="font-medium text-slate-900 mb-2">{t('profileCertifications')}</h4>
            ...
            <div className="bg-white/60 backdrop-blur-md p-6 rounded-3xl border border-white/40 shadow-sm hover:shadow-lg transition-all">
              <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                <Clock className="h-5 w-5" /> {t('profileWorkingHours')}
              </h3>
